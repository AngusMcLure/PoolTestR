...
getPrevalence.glmerMod <- function(model, newdata = NULL, re.form = NULL, all.negative.pools = 'zero',...){
  ...
  for(nge in 1:length(GroupEffectTerms)){
    ...
    ## This section is not working - need to be clear on what it's trying to do to fix
    # Prev should have three columns: Estimate, ..zeroest, and ..allnegative
    
    ## Columns:
    # Estimate = output from meanlinknormal()
    
    # ..allnegative = PredDataSub$..allnegative
    # PredData generated by preparePredictionData()
    ...
    # ..zeroest = new column created when generating Prev dataframe
    # Depends on function input value for all.negative.pools AND values of PredDataSub$..allnegative
    # Boolean value
    # TRUE when all.negative.pools == 'zero' AND PredDataSub$..allnegative == TRUE
    ...
    Prev <- data.frame(Estimate = Vectorize(meanlinknormal)(eta, sds, list(invlink)),
      ...
      select(!('..allnegative')) # ..allnegative in PredDataSub, so remove from Prev
    
    pred <- PredDataSub %>%
      bind_cols(Prev) %>%
      mutate(Estimate = ifelse(.data$..zeroest, 0, .data$Estimate)) %>% # Update zero estimates
